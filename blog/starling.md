...
We introduce Starling-7B-alpha, an open-source chatbot trained by RLAIF with GPT-4 labeled ranking data. Starling-7B-alpha is fine-tuned from [Openchat-3.5](https://huggingface.co/openchat/openchat_3.5) with [Mistral-7B-0.1](https://huggingface.co/mistralai/Mistral-7B-v0.1) as the base model, Starling-7B-alpha achieves 8.09 in MT Bench with GPT-4 as a judge, surpassing all existing models except for GPT-4 and GPT-4 Turbo. The [ranking dataset](https://github.com/lm-sys/FastChat) and [model](), along with an online [demo](https://chat.lmsys.org), are publicly available for non-commercial use.
